...
public class DekripResepBookmarked extends AsyncTask<Void, Void, ModelListTemp[]>{

    private static Context context;
    private static String key = "";
    private static String[] listBookmarkedAlamat = null;
    ...
    public DekripResepBookmarked(Context context, String key, String[] listBookmarkedAlamat){
        DekripResepBookmarked.context = context;
        DekripResepBookmarked.key = key;
        DekripResepBookmarked.listBookmarkedAlamat = listBookmarkedAlamat;
    }
    ...
    protected ModelListTemp[] doInBackground(Void... params) {
        ModelListTemp[] resepModel;
        String[] lines = null;
        String gambar = "";
        resepModel = new ModelListTemp[listBookmarkedAlamat.length];
        for(int i = 0 ; i<listBookmarkedAlamat.length; i++){
            SecretKey baru =  new SecretKeySpec(Base64.decode(key, Base64.DEFAULT),
                    0, Base64.decode(key, Base64.DEFAULT).length, "AES");
            try {
                InputStream inputStream = context.getAssets().open(Constant.namaFolderResep + "/" + listBookmarkedAlamat[i]);
